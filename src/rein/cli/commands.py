...
def cmd_review(args: argparse.Namespace) -> int:
    ...
    if args.stdin and args.diff:
        print("rein: --stdin and --diff both read stdin; use one.", file=sys.stderr)
        return 1

    if args.stdin:
        text = sys.stdin.read()
        findings = _helpers._collect_stdin_findings(
            text, args.filename or "stdin.py", config.custom_rules, profile
        )
    elif args.diff:
