...
# A dotted code reference (module.attr, pkg.Type, host.name) - not a literal
# secret. Each segment is an identifier word; real credentials are opaque runs
# or base64/hex, whose segments start with a digit or carry +/=-, so they never
# match. This keeps a type annotation like `t: vscode.CancellationToken` from
# reading as `t = <secret>`. Single (undotted) identifiers are deliberately NOT
# excluded: an unquoted `api_token=abcd1234` in YAML/.env can be a real secret.
...
def _looks_secret(value: str) -> bool:
    ...
    if _DOTTED_IDENT_RE.match(value):
        return False  # a dotted code reference (module/type/host), not a secret
