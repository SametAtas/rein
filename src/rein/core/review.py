...
def review(
    text: str, path: str | None = None, policy: Policy | None = None, *, domain: Domain
) -> ReviewResult:
    """Run guardrails over an artifact's text and judge the result.

    Runs the given domain to gather findings (the caller injects it; the code
    adapter passes code_domain). Commit hygiene is a different artifact and is
    intentionally not included here.
    """
    findings = domain(text, path)
...
def review_diff_findings(
    new_text: str, diff_text: str, path: str | None = None, *, domain: Domain
) -> list[Finding]:
    ...
    findings = domain(new_text, path)
    return [f for f in findings if f.line is not None and f.line in added]


def review_diff(
    new_text: str, diff_text: str, path: str | None = None, policy: Policy | None = None, *, domain: Domain
) -> ReviewResult:
    ...
    return ReviewResult.from_findings(
        review_diff_findings(new_text, diff_text, path, domain=domain), policy
    )
